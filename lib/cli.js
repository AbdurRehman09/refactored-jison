...
cli.main = function cliMain(opts) {
    ...
    function processInputFile () {
        ...
        console.log("[ [34mDEBUG [0m] Attempting to write parser to:", opts.outfile);
        console.log("[ [34mDEBUG [0m] Length of generated parser string:", parser ? parser.length : 'undefined');
        fs.writeFileSync(opts.outfile, parser);
        console.log("[ [34mDEBUG [0m] Successfully wrote parser to:", opts.outfile);
    }
...
cli.generateParserString = function generateParserString(opts, grammar) {
    ...
    var Generator = jison.Jison.Generator;
    var generator = new Generator(grammar, settings);
...
cli.processGrammars = function processGrammars(file, lexFile, jsonMode) {
    ...
    console.log("[[34mDEBUG[0m] Raw grammar content in cli.processGrammars:\n", file.substring(0, 200) + (file.length > 200 ? "..." : "")); // Log first 200 chars
    ...
    try {
        ...
        console.error("[[31mERROR[0m] Error parsing JISON grammar:", e);
        throw new Error('Could not parse jison grammar: ' + e.message);
    }

    console.log("[[34mDEBUG[0m] Parsed BNF from ebnfParser:\n", JSON.stringify(grammar.bnf, null, 2));
    ...
    try {
        ...
        console.error("[[31mERROR[0m] Error parsing LEX grammar:", e);
        throw new Error('Could not parse lex grammar: ' + e.message);
    }
    return grammar;
};
